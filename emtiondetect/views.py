...
def get_emotion_image_url(text_input):
    
    # Call get_response from emotion.py to get the response
    emotion_response = get_emotion(text_input)
    
    # Extract image URL from the response
    image_url = extract_image_url(emotion_response)  # Define this function
    
    return image_url

def extract_image_url(emotion_response):
    # Split the response to extract the image URL
    lines = emotion_response.split('\n')
    image_url = None
    for line in lines:
...
def get_image_url(request):
    if request.method == 'POST':
        user_message = request.POST.get('userMessage')  # Get user input text
        image_url = get_emotion_image_url(user_message)  # Call get_image_url function
        return JsonResponse({'image_url': image_url})  # Return the image URL as JSON response